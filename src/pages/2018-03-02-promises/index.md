...
path: "/blog/2018/promises-in-js.html"
tags:
    - javascript
    - promises
---

**Recently I receive book recomendation. I started reading it - awesome and deep explanation of how
javascript works. But there was one chapter in this book series about Promises that I cannot handle.
Examples written there were not enough for me - I decided to play with Promises on my own.**

Of course this books is [You Don't Know JS](https://github.com/getify/You-Dont-Know-JS). If you didn't
read it before reading this blog post I highly encourage you to read those book series. This blog post
is mean to be a addition to chapter about [promises](https://github.com/getify/You-Dont-Know-JS/blob/master/async%20%26%20performance/ch3.md).
...
At the begging I added some simple utlity functions that allows me to have my code reused. The most
important part here is `race` declaration.

What is `Promise.race` doing? I waits for first of provided promises to be resolved and `console.log`
this value.
...
It will wait until all promises provided to `all` call are resolved and logs: `My name is krzysztof zuraw`.
Here is a tricky part - what if `fetchLastName` response comes first before `fetchName`? As it turns
out order of Promises added to `all` call is preserved.

Simillary as in `Promise.race` throwing an error results in stopping at first rejection:
